...
TEST_F(PieceTest, GetPossibleMovesReturnsCorrectMoves) {
    ...
    EXPECT_NE(std::find(moves.begin(), moves.end(), std::make_pair(2, 0)), moves.end());
    EXPECT_NE(std::find(moves.begin(), moves.end(), std::make_pair(3, 0)), moves.end());

    // Test for a black knight at (7, 1)
    piece = Piece::makePiece(PieceType::Knight, Color::Black);
    moves = Piece::getPossibleMoves(piece, 7, 1);
    EXPECT_EQ(moves.size(), 3);
    EXPECT_NE(std::find(moves.begin(), moves.end(), std::make_pair(5, 0)), moves.end());
    EXPECT_NE(std::find(moves.begin(), moves.end(), std::make_pair(5, 2)), moves.end());
    EXPECT_NE(std::find(moves.begin(), moves.end(), std::make_pair(6, 3)), moves.end());
    ...
    EXPECT_EQ(moves.size(), 27);  // Queen can move in 8 directions, but some moves are blocked by the board edges
}
